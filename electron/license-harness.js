...
const licenseService = require('./tmp/licenseService')

const STORAGE_FILE = 'license-state.bin'
const USER_DATA = path.join(__dirname, '..', '.electron-user-data')
...
async function run() {
  try {
    ...
    // Free plan quotas reset for next tests
    await licenseService.deactivateLicense()
    ...
    for (let i = 1; i <= 5; i += 1) {
      ...
      if (rec.usage.extensionInstalls !== i) {
        extensionOk = false
        break
      }
    }
    ...
    // Activate PRO and verify unlimited behavior
    ...
    // Refresh state test: expire current plan and refresh
    const raw = await loadRawState()
    const oldExpiry = new Date(Date.now() - 1000 * 60 * 60 * 24).toISOString()
    raw.expiresAt = oldExpiry
